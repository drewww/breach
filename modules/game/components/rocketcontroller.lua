...
function RocketController:act(level, actor)
   if not self.vector then
      ...
      if player then
         ...
         local destination = actor:getPosition() + vector * 10
         ...
         prism.logger.info("path length: ", #self.path)
      end
   end

   -- no checks -- just GO. let the action work it out.
   local next = table.remove(self.path, 1)

   if not next then
      return prism.actions.Die(actor)
   end

   prism.logger.info("move to ", next, " from ", actor:getPosition())
   return prism.actions.Fly(actor, next)
end
